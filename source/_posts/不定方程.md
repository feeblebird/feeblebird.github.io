...
* #### 分析
  ...
  ![分析](https://i.loli.net/2020/08/07/6AJIOLq4l5o3GFr.png)
...
```cpp
 ...
 int main()
 {
     ...
     for(int i = 1;i <= t;i ++)
     {
         ...
         {
             mi = max(abs(x0),abs(y0));
         }
         k = y0/(a/gcd);
         for(ll m = k-3;m <= k+3;m ++)
         {
             ll nansx = x0 + m*b/gcd;
             ll nansy = y0 - m*a/gcd;
             if(nansx*nansy<=0&&abs(nansx)+abs(nansy)<mi)  //注意等于零的情况的考虑
             {
                 mi = abs(nansx) + abs(nansy);
             }
         }
         k = (y0-x0)*gcd/(a+b);
         for(ll m = k - 3;m <= k+3;m ++)
         {
             ll nansx = x0 + m*b/gcd;
             ll nansy = y0 - m*a/gcd;
             if(nansx*nansy>=0&&max(abs(nansx),abs(nansy))<mi) //注意等于零的情况的考虑
             {
                 mi = max(abs(nansx),abs(nansy));
             }
         }
         printf("%lld\n",mi);
     }
...
> 注意在枚举k值附近的点的时候，要考虑nansx * nansy == 0的情况，之前就是一直卡在这里出错。